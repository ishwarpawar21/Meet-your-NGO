...
class Home extends CI_Controller {
	...
	public function index($search='all',$searchdate='')
	{
	  ...
	  /*pagingnation*/
	  if($search!='all' && $search!='higher-lower-price' && $search!='lower-higher-price' && $search!='lower-higher-review' && $search!='higher-lower-review' && $search!='date' && $search!='hot-coupon' && $search!='latest')
	  {
		  $this->db->where('tbl_category_master.category_slug',$search);	  
		  $this->db->join('tbl_category_master','tbl_category_master.category_id=tbl_coupon_master.coupon_cat_id'); 	  
	  }
	  /*search of price*/
	  if($search!='all' && $search=='higher-lower-price' && $search!='date' && $search!='hot-coupon')
	  {$this->db->order_by('tbl_coupon_master.order_by_price','DESC');}
	  else if($search!='all' && $search=='lower-higher-price' && $search!='date' &&  $search!='hot-coupon')
	  {$this->db->order_by('tbl_coupon_master.order_by_price','ASC');}
	  /*search of review */
	  else if($search!='all' && $search=='higher-lower-review' && $search!='date' &&  $search!='hot-coupon')
	  {$this->db->order_by('tbl_coupon_master.product_reviews_digit','DESC');}
	  else if($search!='all' && $search=='lower-higher-review' && $search!='date' && $search!='hot-coupon')
	  ...
	  /*search of review */
	  /*searchin of heigher coupon price to lower coupon price */
	  ...
	  $productcoupon=$this->master_model->getRecords('tbl_coupon_master',array('tbl_coupon_master.coupon_status'=>'1'));
	 //echo  $this->db->last_query();
	  $config['base_url']=base_url().'category/'.$search.'/';
	  $config['total_rows']=count($productcoupon);
	  if($search=='date')
	  {
	    $config['uri_segment']=5;
	    $page_link = ($this->uri->segment(5)) ? $this->uri->segment(5) : 0;
	  }
	  else
	  {
	  ...
	  $data['product_coupon']=$this->master_model->getRecords('tbl_coupon_master',array('tbl_coupon_master.coupon_status'=>'1'),'','',$page,$config['per_page']);
	  /*echo count($data['product_coupon']);
	  echo $this->db->last_query();*/
